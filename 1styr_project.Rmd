...
redcap$PID <- as.factor(redcap$participant_id);
redcap.desi$PID <- as.factor(redcap.desi$PID);
PGNG$PID <- as.factor(PGNG$PID);
...
redcap.clean <- redcap[redcap$PID %in% demo$PID,]
...
#Drop unused levels
redcap.clean$PID <- droplevels(redcap.clean$PID)
...
#Over here, it only accounts for the prompts that have regulation!
